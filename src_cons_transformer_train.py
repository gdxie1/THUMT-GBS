...
def decoding_graph(features, state, mode, params):
    ...
    # loss = tf.Print(loss, [tf.reduce_sum(ce*tgt_mask), tf.reduce_sum(tgt_mask)], "tf.reduce_sum(ce*tgt_mask), tf.reduce_sum(tgt_mask)", 1000, 1000)

    #########################################
    # 开始计算 alignment的loss  要形成 tgt--->src的对齐矩阵
    layer_weight = decoder_weight[5]  # layer5 128 8 32 37:128是batch_size，每一层都是针对batch进行操作，所以每一层的结果是一个batch的结果
    att_matrix = layer_weight[:, 5]  # only keep the 5th head: 128 32 37

    att_matrix_shape = tf.shape(att_matrix)
    # att_matrix_shape = tf.Print(att_matrix_shape, [att_matrix_shape, tf.shape(features["source"])],
    #                             "att_matrix_shape, shape-of-features[\"source\"]", 1000, 1000)
    # att_matrix
    src_mask = tf.sequence_mask(src_len, maxlen=tf.shape(features["source"])[1])  # 有词存在的位置是True, padded的位置是False: N SL, features["source"]): [2 10]
    ...
    padded_mask = features["align_mask"]  # mask中后面的0就是padded mask
    align_tgt = features["align_tgt"]  # 1 2 3 .xxx 0 0 0
    align_src = features["align_src"]  # 0 2 4 .xxx 0 0 0
    ...
    # 由于alignment info 并不是是按照词的顺序排列，所以索引并非按照次序索引，例如 [[0 0 0]][[0 1 2]] [[0 3 2]][[0 4 3]][[0 2 6]] 虽然索引没有按照顺序，但确定能唯一索引到某个位置
    indices = tf.concat([batch_indices, indices], 1)   # [[0 x x] [0 x x ] ... [1 x x ] [1 x x ]....]
    padded_mask = tf.reshape(padded_mask, [-1])

    # indices = tf.Print(indices, [indices, padded_mask], "indices, padded_mask", 100, 10000)

    index_indices = tf.where(padded_mask)
    indices = tf.gather(indices, index_indices)
    # indices = tf.Print(indices, [indices], "indices, after masked", 100, 10000)

    indices_new = tf.cast(indices, dtype=tf.int64)
    indices_new = tf.squeeze(indices_new, 1)
    indicator_shape = tf.cast(att_matrix_shape, dtype=tf.int64)

    # indices_new = tf.Print(indices_new, [align_tgt], "align tgt", -1, 10000)
    # indices_new = tf.Print(indices_new, [align_src], "align-src", -1, 10000)
    # indices_new = tf.Print(indices_new,
    #                        [tf.reduce_max(align_tgt), tf.reduce_max(align_src),
    #                         indicator_shape, indices_new], "max-tgt, max-src, indicator_shape, indices_new", -1, 10000)

    indicator = tf.sparse_to_dense(indices_new, indicator_shape, 1.0, validate_indices=False)
    # indicator = tf.Print(indicator, [indicator], "indicator", 100, 10000)
    indicator_src_sum = tf.reduce_sum(indicator, -1, keep_dims=True)
    ...
    # indicator = tf.Print(indicator, [indicator], "indicator after normalized", 100, 10000)
    standard_align = tf.gather_nd(indicator, indices)
    decoded_align = tf.gather_nd(att_matrix, indices)
    # decoded_align = tf.Print(decoded_align , [decoded_align], "decoded_align", 100, 10000)
    # standard_align = tf.Print(standard_align, [standard_align], "standard_align", 100, 10000)
    # align_loss_model = 'square-mean'
    if params.align_loss_model == 'square-mean':
        _err = tf.square(tf.subtract(decoded_align, standard_align))  # N Tl Sl
    else:
        mini_val_mask = tf.cast(tf.equal(decoded_align, 0.0), dtype=tf.float32) * 0.00000001  # 先进行上一步，再进行下一步
        decoded_align = decoded_align + mini_val_mask
        _err = -standard_align * tf.log(decoded_align)
    ...
    # loss = tf.Print(loss, [tf.reduce_sum(_err), tf.reduce_sum(padded_mask)], "_err, sum of padded_mask", 100, 1000)

    align_loss = tf.reduce_sum(_err)/tf.cast(tf.reduce_sum(padded_mask), dtype=tf.float32)  # batch内的句子平均
    ...
    # start_step = tf.constant(50000, dtype=tf.float32)
    # end_step = tf.constant(350000, dtype=tf.float32)
    #
    ...
    decay_ratio = 0.05
    # loss = tf.Print(loss, [loss, align_loss], 'nll loss, align loss', -1, 1000)
